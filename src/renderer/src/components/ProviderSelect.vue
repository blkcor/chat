      ...
      <SelectContent side="bottom" position="popper" align="start" :side-offset="6" class="min-w-[220px] rounded-xl shadow-lg border border-border-color
        ...
        <SelectViewport class="p-2">
          <template v-for="(provider, pIndex) in providers" :key="provider.id">
            ...
            <!-- 模型列表 -->
            <SelectGroup>
              <SelectItem v-for="model in provider.models" :key="model" :value="model" class="relative flex items-center h-9 px-3 rounded-lg cursor-pointer text-sm text-primary
                ...
                <SelectItemText class="pl-5">{{ model }}</SelectItemText>
              </SelectItem>
            </SelectGroup>
