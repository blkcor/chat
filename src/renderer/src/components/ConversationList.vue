  ...
  <div>
    <ul class="list-none p-0 m-0 flex flex-col gap-2">
      <li
        ...
        :key="item.id"
        class="list-item"
        :class="{ selected: conversationId === item.id }"
      >
        <router-link :to="`/conversation/${item.id}`">
          ...
          <h3
            class="text-[0.9rem] font-medium mx-0 mt-1 mb-0.5"
          ...
          <div class="flex items-center mt-1">
            ...
            ></div>
            <p
              class="text-[0.75rem] m-0 truncate"
...
const selectedConversationId = ref<string | null>(conversationId) // Default select the first one
